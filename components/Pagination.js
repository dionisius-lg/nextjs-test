...
import { PaginationDots, PaginationRange, PaginationInfo } from "utils/pagination";

export default function Pagination({pageChange, total, limit, paging, sibling = 1 }) {
    let numDots = 0
            ...
            <nav className={ classnames('float-right', {'d-none': current === 0 || pageRange.length < 2}) }>
                <ul className="pagination mb-0 pb-0">
                    <li key="previous" className={ classnames('page-item', {'d-none': current === first}) }>
                        <button type="button" className="page-link" onClick={() => pageChange(previous)}>Prev</button>
                    </li>
